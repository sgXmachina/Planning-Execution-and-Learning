...
{	
	...
	bool board_cell_state::check_goal_reached() const
	{
	...
	bool board_cell_state::get_valid() const
	{
		return valid_;
	}
	...
	int board_cell_state::get_xy(const unsigned int x, const unsigned int y) const
	{
		return grid_[x][y];
	}

	void board_cell_state::print_grid() const
	{	
...
//Set the parent of the current state
	...
	//Get the parent
	std::shared_ptr<board_cell_state> board_cell_state::get_parent() const
	{
		return parent_;
	}

	//Return the current grid state
	grid_type board_cell_state::get_grid() const
	{
		return grid_;
	}
